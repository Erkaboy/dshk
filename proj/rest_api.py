...
def dshk_rest_api(request,xizmat='',obj_id='obj_id'):
	...
	if xizmat=='geologik-rayonlashtirish-umumiy-ruyxati':
	...
	elif xizmat=='hududlarni-arxitekturaviy-rejalashtirishni-tashkillashtirish-loyihalari-asosida-qilingan-funksional-zonalar-umumiy-ruyxati':
		fza_data= FunkApots.objects.filter(~Q(status=4)).values(
		'id',
		)
		return JsonResponse({'data':list(fza_data)},safe=False)
	

	elif xizmat=='bitta-hududlarni-arxitekturaviy-rejalashtirishni-tashkillashtirish-loyihalari-asosida-qilingan-funksional-zona-haqida-tuliq-malumot':
		fza_data= FunkApots.objects.filter(id=str(obj_id))
		fza_json= serializers.serialize('geojson',fza_data,geometry_field='wkb_geometry',fields={				
				'funk_zone',
				'vil_id',
				'zonalarning_nomi',
		...
		if fza_data.first():
			obj_id=fza_data.first().apot_id.pk
		else:
			obj_id=-1

		apot_data= Apots.objects.filter(pk=str(obj_id)).values(
				'fuqarolar_yiginlari',
		...
		return JsonResponse({'main_data':json.loads(fza_json),'additional_data':list(apot_data)},safe=False)
		...
		return HttpResponse('Manzil xato yozilgan !')
